...
def evaluate_classifier(classifier, X_train, X_test, y_train, y_test):
    # Inner function to include both fitting and prediction
    def fit_and_predict():
        classifier.fit(X_train, y_train)
        return classifier.predict(X_test)

    # Measure execution time for fitting and predicting
    ...
    # Return all the metrics excluding memory usage
...
for classifier in classifiers:
    ...
    # Определяем, какие данные использовать в зависимости от ресемплинга
    X_train_current = X_train_flat_resampled if resampling_done else X_train_flat
    y_train_current = y_train_resampled if resampling_done else y_train
    ...
    exec_time, max_mem_usage, precision, accuracy, f1_score_val, roc_auc_macro, roc_auc_micro, confusion = \
        evaluate_classifier(classifier, X_train_current, X_test_flat, y_train_current, y_test)
