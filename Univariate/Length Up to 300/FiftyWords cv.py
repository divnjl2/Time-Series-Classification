...
n_folds=5
def evaluate_classifier_timeseries_split(classifier, X, y, n_splits=5):
...
for classifier in classifiers:
    ...
    avg_confusion_matrix, exec_time, max_mem_usage, accuracy, precision, f1_score_val, roc_auc_macro, roc_auc_micro = evaluate_classifier_timeseries_split(classifier, X_train_flat_resampled, y_train_resampled, n_folds)
    ...
    results["Confusion Matrix"].append(avg_confusion_matrix)
...
for classifier in results["Classifier"]:
    ...
    print(f"  ROC-AUC Score (Micro): {results['ROC-AUC Score (Micro)'][index]:.4f}\n")
